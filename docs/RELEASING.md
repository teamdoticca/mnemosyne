...
The source tree prepares version `0.1.3`; it is not published merely by changing the project version. Never overwrite an existing NuGet version or move a published tag.
...
1. Update `Version` in `src/Mnemosyne/Mnemosyne.csproj` using three-component SemVer. A patch preserves API compatibility; a breaking 0.x change needs a minor bump and migration notes.
...
6. Create and push the matching tag, for example `v0.1.3`, on the reviewed `main` commit. The tag run validates it and publishes to GitHub Packages.
...
9. For the next release, advance the source version and, after reviewing compatibility, update `PackageValidationBaselineVersion` to the release just published. The initial baseline is `0.1.2.17`.
